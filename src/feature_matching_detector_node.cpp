...
public:
    ...
    : nh_private_("~"), it_(nh_)
  {
    ...
    nh_private_.param("model", model_filename_, std::string("model.yaml"));

    ROS_INFO_STREAM("Settings:" << std::endl
        << "  model               : " << model_filename_ << std::endl
  ...
  void loadModel()
  {
    ...
    if (object_pose_estimation::features_io::loadFeatures(
      ...
      const ros::Time& stamp, const std::string& camera_frame_id)
  {
    ...
    detection_msg.object_pose = pose_msg.pose;
